...
func (app *Application) createTodoEntry(appID string, orgID string, userID string, todo *model.TodoEntry) error {
	return app.storage.PerformTransaction(func(context storage.TransactionContext) error {
		...
		err = app.notifications.SendNotification([]model.NotificationRecipient{{UserID: createTodoEntry.UserID}}, &topic, "TODO Reminder", createTodoEntry.Title, createTodoEntry.AppID, createTodoEntry.OrgID, &dueDateTime, map[string]string{
		...
		err = app.notifications.SendNotification([]model.NotificationRecipient{{UserID: createTodoEntry.UserID}}, &topic, "TODO Reminder", createTodoEntry.Title, createTodoEntry.AppID, createTodoEntry.OrgID, &reminderDateTime, map[string]string{
			"type":        "wellness_todo_entry",
			"operation":   "todo_reminder",
			"entity_type": "wellness_todo_entry",
			"entity_id":   createTodoEntry.ID,
...
func (app *Application) updateTodoEntry(appID string, orgID string, userID string, todo *model.TodoEntry, id string) error {
	return app.storage.PerformTransaction(func(context storage.TransactionContext) error {
		...
		err = app.notifications.SendNotification([]model.NotificationRecipient{{UserID: updateTodoEntry.UserID}}, &topic, "TODO Reminder", updateTodoEntry.Title, updateTodoEntry.AppID, updateTodoEntry.OrgID, nil, map[string]string{
			"type":        "wellness_todo_entry",
			"operation":   "todo_reminder",
			"entity_type": "wellness_todo_entry",
...
func (app *Application) deleteTodoEntry(appID string, orgID string, userID string, id string) error {
	...
	return app.storage.PerformTransaction(func(context storage.TransactionContext) error {
		...
		err = app.notifications.SendNotification([]model.NotificationRecipient{{UserID: userID}}, &topic, "TODO Reminder", title, appID, orgID, nil, map[string]string{
			"type":        "wellness_todo_entry",
			"operation":   "todo_reminder",
			"entity_type": "wellness_todo_entry",
			"entity_id":   id,
	...
	return app.storage.PerformTransaction(func(context storage.TransactionContext) error {
		...
		if len(todos) > 0 {
			for _, todo := range todos {
				err := app.notifications.SendNotification([]model.NotificationRecipient{{UserID: todo.UserID}}, &topic, "TODO Reminder", todo.Title, todo.AppID, todo.OrgID, nil, map[string]string{
					"type":        "wellness_todo_entry",
					"operation":   "todo_reminder",
					"entity_type": "wellness_todo_entry",
					"entity_id":   todo.ID,
